    ...
    for (var p = 0; p < st.pieces[pieceType].length; p++) {
          ...
          if (moveActions.length > 0) {
            ...
            if (upActions.length === 0) {
              ...
              console.log("Piece ID ", piece.id, " now has heightBoost " + tempState.heightBoosts[piece.id]);
            }
          }
    }
...
Rule.prototype.place = function (state, number, type, position) {
  for (var i = 0; i < number; i++) {
    ...
    state.heightBoosts[state.nextPieceID] = 0;
    state.nextPieceID++;
  }
};
...
Rule.prototype.up = function (state, piece, steps) {
  ...
  if (state.heightBoosts[piece.id] === undefined)
    state.heightBoosts[piece.id] = 0;
  state.heightBoosts[piece.id] += steps;
};
...
Rule.prototype.nextTurn = function (match) {
  ...
  model.State.resetHeightBoosts(game.state);
