...
Rule.prototype.rollDice = function(game, values) {
  ...
  console.log("Pieces with potential moves:", weight)
  ...
  console.log('Playable dice:', weight.playableMoves);
...
Rule.prototype.calculateMoveWeights = function(state, movesLeft, pieceType, rootPiece, stopAtMax, moveType) {
  ...
  function calculateBranchWeights(st, moves, id, branchSum, level, branchMoves, moveType) {
    ...
    function applyTempActions (actions,tempState,id,branchSum,shouldStop,calculateBranchWeights,level,moveType) {
      ...
      console.log('Applying actions', actions, " to piece ", actions[0].piece.id);
    ...
    for (var p = 0; p < st.pieces[pieceType].length; p++) {
          ...
          // If the piece can be moved, apply it, otherwise try an UP
          ...
          if (moveActions.length > 0) {
            ...
            console.log("Piece ID ", piece.id, " is now at position " + model.State.getPiecePos(tempState, piece));
          }
          else {
            console.log('Cannot complete that MOVE action for this piece');
            var upActions = self.getMoveActions(tempState, piece, steps, model.MoveActionType.UP);
            if (upActions.length === 0) {
              console.log('No UP (or MOVE) actions for this piece');
            }
            else {
              applyTempActions(upActions,tempState,id,branchSum,shouldStop,calculateBranchWeights,level,model.MoveActionType.UP);
              console.log("Piece ID ", piece.id, " now has heightBoost " + tempState.boostedHeight[piece.id]);
            }
          }
    }
...
Rule.prototype.applyMoveActions = function(state, actionList) {
  for (var i = 0; i < actionList.length; i++) {
    ...
    console.log("Action:", action, action.piece);
...
Rule.prototype.place = function (state, number, type, position) {
  for (var i = 0; i < number; i++) {
    ...
    state.boostedHeight[state.nextPieceID] = 0;
    state.nextPieceID++;
  }
};
...
Rule.prototype.up = function (state, piece, steps) {
  ...
  if (state.boostedHeight[piece.id] === undefined || state.boostedHeight[piece.id] === 0)
    state.boostedHeight[piece.id] = state.points[fromPos].length + steps;
  else
    state.boostedHeight[piece.id] += steps;
};
...
Rule.prototype.nextTurn = function (match) {
  ...
  model.State.resetboostedHeight(game.state);
